...
static void startup(GtkApplication *app) {
	...
	g_signal_connect_swapped(hideControls, "clicked", G_CALLBACK(hide_controls), NULL);

	showPlayer = gtk_button_new_from_icon_name("sidebar-show-symbolic");
	gtk_widget_set_halign(showPlayer, GTK_ALIGN_END);
	g_signal_connect_swapped(showPlayer, "clicked", G_CALLBACK(show_main_window), NULL);

	closeButton = gtk_button_new_from_icon_name("window-close-symbolic");
	gtk_widget_set_halign(closeButton, GTK_ALIGN_END);
	g_signal_connect_swapped(closeButton, "clicked", G_CALLBACK(close_main_window), NULL);
	...
	gtk_box_append(GTK_BOX(allBox), APPNAME);
