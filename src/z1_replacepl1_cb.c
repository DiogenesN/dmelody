...
#define NAMEOFREPLACEFUNC	replacepl1_cb
#define NAMEOFWAITFUNC		waiting_populating_r1_files

//////////////////////// replace playlist 1 action ////////////////////////////////////////

gboolean NAMEOFWAITFUNC(gpointer data) {
	(void)data;
...
static void NAMEOFREPLACEFUNC() {
	...
	replacenthline(pathToConfigDmelody, LINENUMBERTOREPLACE, folderNameLineToReplace);
	...
	get_strs_from_substrs_write_to_conf(fullPathToSelDir, extArr, PATHTOPLAYLIST, "w");

	g_timeout_add_seconds(1, NAMEOFWAITFUNC, NULL);
}
