...
#define LINENUMBERTOREPLACE2	9
#define PLAYLISTLABEL2			playlist2
#define NAMEOFREPLACEFUNC2		replacepl2_cb
#define PATHTOPLAYLIST2			pathToPlaylist2
#define PLAYLISTNAME2			"playlist2name="
#define NAMEOFWAITFUNC2			waiting_populating_r2_files
#define PATHTONUMBERSFILE2		pathToConfigDmelodyDirNumbersFile2
#define PATHTOARTISTSFILE2		pathToConfigDmelodyDirArtistsFile2
#define PATHTOTITLESFILE2		pathToConfigDmelodyDirTitlesFile2
#define PATHTOALBUMSFILE2		pathToConfigDmelodyDirAlbumsFile2
#define PATHTODURATIONSFILE2	pathToConfigDmelodyDirDurationsFIle2

//////////////////////// replace playlist 2 action ////////////////////////////////////////

gboolean NAMEOFWAITFUNC2(gpointer data) {
	(void)data;
	...
	FILE *playlist = fopen(PATHTOPLAYLIST2, "r");
	FILE *numbersF = fopen(PATHTONUMBERSFILE2, "w");
	FILE *artistsF = fopen(PATHTOARTISTSFILE2, "w");
	FILE *titlesF = fopen(PATHTOTITLESFILE2, "w");
	FILE *albumsF = fopen(PATHTOALBUMSFILE2, "w");
	FILE *durationsF = fopen(PATHTODURATIONSFILE2, "w");
	...
	strcpy(coverArtPathConfName, "playlist2cover=");
	strcat(coverArtPathConfName, coverArtFullPath);
	replacenthline(pathToConfigDmelody, 13, coverArtPathConfName);
...
static void NAMEOFREPLACEFUNC2() {
	...
	snprintf(folderNameLineToReplace, sizeof(folderNameLineToReplace), PLAYLISTNAME2"%s\n",
																		folderNameForPlaylistLabel);
	replacenthline(pathToConfigDmelody, LINENUMBERTOREPLACE2, folderNameLineToReplace);
	...
	PangoLayout *layout = gtk_widget_create_pango_layout(PLAYLISTLABEL2, NULL);
	pango_layout_set_attributes(layout, attr_list);
	gtk_label_set_attributes(GTK_LABEL(playlist1), NULL);
	gtk_label_set_attributes(GTK_LABEL(PLAYLISTLABEL2), attr_list);
	gtk_label_set_attributes(GTK_LABEL(playlist3), NULL);
	gtk_label_set_label(GTK_LABEL(PLAYLISTLABEL2), folderNameForPlaylistLabel);
	...
	if (coverArt != NULL) {
		...
		snprintf(coverArtPathConfName, sizeof(coverArtPathConfName), "playlist2cover=%s\n", coverArt);
		replacenthline(pathToConfigDmelody, 13, coverArtPathConfName);
	}
	else {
	...
	gchar *lastTabClicked = "lastTabClicked=2";
	...
	get_strs_from_substrs_write_to_conf(fullPathToSelDir, extArr, PATHTOPLAYLIST2, "w");

	g_timeout_add_seconds(1, NAMEOFWAITFUNC2, NULL);
}
