...
// action on first tab clicked
void on_playlist1_l_click() {
	...
	if (nrOfElements < 1 && get_number_of_lines(pathToPlaylist1) < 1) {
		return;
	}
	if (get_number_of_lines(pathToPlaylist1) == 0) {
		return;
	}
	...
	// playlist has tracks then do the following
